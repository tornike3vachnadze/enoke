...
type Props = {
  ...
  onSelectBG: (data: ColorItem) => void;
...
const TColorPicker = ({ onSelect, item, position, onSelectBG }: Props) => {
  ...
  useEffect(() => {
    if (item && item.type == "text" && item.content.style) {
      const lightIndex = LightColor.findIndex(
        (a) => a.color == item.content.style?.color
      );
      setSelected(lightIndex);
    }
    if (item && item.type == "text" && item.content.style) {
      const darkIndex = DarkColor.findIndex(
        (a) => a.color == item.content.style?.background
      );
      setSelectedBg(darkIndex);
    }
    return () => {};
  }, []);
      ...
      >
        <View
          style={[
            styles.pressable,
            {
              borderColor: selectedBg == index ? "black" : "transparent",
            },
          ]}
        >
          <View
